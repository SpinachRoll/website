...
document.getElementById("calc_cmb_lvl_btn").addEventListener("click", function() {
	...
	var cmb;
	if (melee_cmb > range_cmb) {
		cmb = melee_cmb;
		next_lvl = Math.max(Math.ceil(cmb), Math.floor(cmb + 1));
		adsh_left = Math.ceil((next_lvl - cmb) / .25);
		mp_left = Math.ceil((next_lvl - cmb) / .125);
		cmb_lvl.innerHTML = "Your Combat Level is " + cmb + ". <br/><br/>You are a Melle based account.<br/><br/>" + "You need " + adsh_left + " more Att, Def, Str, or Hits levels, or " + mp_left + " more Mage or Pray levels for your next Combat Level";
	} else {
		cmb = range_cmb;
		next_lvl = Math.max(Math.ceil(cmb), Math.floor(cmb + 1));
